...
def clean_tape_query(df):
    ...
    df['latest_due_date'] = df[['new_due_date', 'due_date']].max(axis=1)
    df['days_dq'] = df['days_delinquent'][df['days_delinquent'] > 0]
    df['days_to_repay'] = df.apply(get_days_to_repay, axis=1)
    ...
    df['currency'][df['currency'] == 'EURO'].str.replace('EURO', 'EUR')
    df['currency'][df['currency'] == 'EURO'].str.replace('POUND', 'GBP')
    df['hs_code_descrip_short'] = df['hs_code_description'].str[0:25]
